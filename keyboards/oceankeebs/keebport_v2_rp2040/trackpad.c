...
uint8_t		stop = 1;
uint8_t		tapTime[2] = {2,0};
uint8_t		tapDistance[2] = {0,200};
uint8_t		swipeDistance[2] = {0,10};
uint8_t		sfGestureEnable[1] = {1};
...
void pointing_device_init(void) { 
	...
	if(is_keyboard_master()){
		...
		i2c_writeReg16(IQS5xx_ADDR<<1, END_WINDOW, &stop, 1, 7);
		//i2c_writeReg16(IQS5xx_ADDR<<1, TapTime_adr, &tapTime[0], 2, 4);
		//i2c_writeReg16(IQS5xx_ADDR<<1, END_WINDOW, &stop, 1, 7);
		//i2c_writeReg16(IQS5xx_ADDR<<1, TapDistance_adr, &tapDistance[0], 2, 4);
		//i2c_writeReg16(IQS5xx_ADDR<<1, END_WINDOW, &stop, 1, 7);
		//i2c_writeReg16(IQS5xx_ADDR<<1, SwipeInitDistance_adr, &swipeDistance[0], 2, 4);
		//i2c_writeReg16(IQS5xx_ADDR<<1, END_WINDOW, &stop, 1, 7);
		//i2c_writeReg16(IQS5xx_ADDR<<1, SFGestureEnable_adr, &sfGestureEnable[0], 1, 4);
		//i2c_writeReg16(IQS5xx_ADDR<<1, END_WINDOW, &stop, 1, 7);
