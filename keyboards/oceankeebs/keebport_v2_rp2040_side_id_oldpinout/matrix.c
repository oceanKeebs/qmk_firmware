...
#define DEBUGMSG_EVERY_COUNT 5000
uint16_t debugCounter = DEBUGMSG_EVERY_COUNT;
...
static bool read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row) {
    ...
    if (debugCounter == DEBUGMSG_EVERY_COUNT) {
        ...
        dprintf("expander0_ID:   %i  expander1_ID:   %i  expander2_ID:   %i  expander3_ID:   %i\n", expander0_ID, expander1_ID, expander2_ID, expander3_ID);
    }
    else debugCounter++;
    ...
    for(uint8_t col_index = 0; col_index < MATRIX_COLS; col_index++) {
        ...
        switch (col_index) {
            case 0 :
                if ((expander0error || (expander0_ID != 0)) && (expander2error || (expander2_ID != 0))) pin_state = 1;
                else if (expander0error || (expander0_ID != 0)) pin_state = port_expander2_col_buffer[0] & (1 << 5);
                else if (expander2error || (expander2_ID != 0)) pin_state = port_expander0_col_buffer[0] & (1 << 5);
            ...
            case 20 :
                ...
                break;
            case 21 :
                if ((expander1error || (expander1_ID != 0)) && (expander3error || (expander3_ID != 0))) pin_state = 1;
                else if (expander1error || (expander1_ID != 0)) pin_state = port_expander3_col_buffer[0] & (1 << 5);
                else if (expander3error || (expander3_ID != 0)) pin_state = port_expander1_col_buffer[0] & (1 << 5);
